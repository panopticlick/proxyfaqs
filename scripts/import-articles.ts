...
function categorizeQuestion(tags: string[]): {
  ...
  if (tagsLower.some((t) => t.includes("residential"))) {
    ...
    tagsLower.some((t) => t.includes("scraper") || t.includes("api"))
  ) {
    ...
    tagsLower.some((t) => t.includes("provider") || t.includes("service"))
  ) {
    ...
    tagsLower.some((t) => t.includes("type") || t.includes("comparison"))
  ) {
    ...
    for (const filePath of filesToProcess) {
      try {
        ...
        if (!article.slug || !article.title || !article.detailed_answer) {
          console.warn(
            `⚠️  Skipping ${path.basename(filePath)}: Missing required fields`,
          );
          totalSkipped++;
          continue;
        }
